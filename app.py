...
app.config['SQLALCHEMY_DATABASE_URI'] = 'postgresql://postgres:040295@localhost:5432/flaskJWT.db'
...
def token_required(func):
    ...
    def decorator(*args, **kwargs):
        ...
        try:
            print(token)
            data = jwt.decode(str.encode(token), app.config["SECRET_KEY"], algorithms=['HS256'])
            print(data)
...
def create_author(current_user):
    data = request.get_json()
    new_author = Authors(name=data['name'], country=data['country'], book=data['book'], booker_prize=True,
...
@token_required
def get_author_details(current_user, author_id):
    ...
    return jsonify({'message': author_data})
